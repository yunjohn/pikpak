...
let sources=Array.isArray(payload.sources)?payload.sources:decodeList(params.get('sources'));const items=Array.isArray(payload.items)?payload.items:decodeList(params.get('items')),subtitles=Array.isArray(payload.subtitles)?payload.subtitles:decodeList(params.get('subtitles'));
document.title=name;
...
if(kind==='video'){
  const shell=document.createElement('section'),video=document.createElement('video'),toolbar=document.createElement('div'),select=document.createElement('select'),speedSelect=document.createElement('select'),audioSelect=document.createElement('select'),subSelect=document.createElement('select'),subtitle=document.createElement('button'),pip=document.createElement('button'),capture=document.createElement('button'),status=document.createElement('div');
  ...
  const preferences=readPreferences(),savePreferences=()=>{try{localStorage.setItem(preferenceKey,JSON.stringify({rate:video.playbackRate,volume:video.volume,muted:video.muted}))}catch{}};
  shell.className='video-shell';toolbar.className='video-tools';video.controls=true;video.autoplay=true;video.playsInline=true;video.volume=preferences.volume;video.muted=preferences.muted;video.playbackRate=preferences.rate;video.style.objectFit='contain';bindProgress(video);status.className='viewer-status';select.title='清晰度';speedSelect.title='播放速度';audioSelect.title='音轨';audioSelect.hidden=true;subSelect.title='在线字幕';subSelect.hidden=!subtitles.length;subtitle.textContent='字幕';pip.textContent='画中画';pip.title='画中画（P）';capture.textContent='截图';let choices=sources.length?sources:[{url,label:'默认'}];const renderChoices=()=>{select.replaceChildren(...choices.map((source,index)=>{const option=document.createElement('option');option.value=String(index);option.textContent=source.label||`线路 ${index+1}`;return option}));select.hidden=choices.length<=1};renderChoices();[.5,.75,1,1.25,1.5,2].forEach(rate=>{const option=document.createElement('option');option.value=String(rate);option.textContent=`${rate}×`;if(rate===preferences.rate)option.selected=true;speedSelect.appendChild(option)});
  ...
  toolbar.append(select,speedSelect,audioSelect,subSelect,subtitle,pip,capture);shell.append(video,toolbar,status);root.replaceChildren(shell);load(0);
  if(subtitles.length&&subtitles[0]?.url){fetch(subtitles[0].url).then(r=>r.ok?r.arrayBuffer():Promise.reject(new Error(`HTTP ${r.status}`))).then(bytes=>window.viewerPayload?.decodeSubtitle?window.viewerPayload.decodeSubtitle(bytes):new TextDecoder().decode(bytes)).then(text=>{applySub(subtitles[0].name,text,0);subSelect.value='0'}).catch(()=>{})}
}else if(kind==='image'){
