...
if(kind==='video'){
  const shell=document.createElement('section'),video=document.createElement('video'),toolbar=document.createElement('div'),select=document.createElement('select'),audioSelect=document.createElement('select'),subSelect=document.createElement('select'),subtitle=document.createElement('button'),capture=document.createElement('button'),status=document.createElement('div');shell.className='video-shell';toolbar.className='video-tools';video.controls=true;video.autoplay=true;video.playsInline=true;bindProgress(video);status.className='viewer-status';select.title='清晰度';audioSelect.title='音轨';audioSelect.hidden=true;subSelect.title='在线字幕';subSelect.hidden=!subtitles.length;subtitle.textContent='字幕';capture.textContent='截图';const choices=sources.length?sources:[{url,label:'默认'}];choices.forEach((source,index)=>{const option=document.createElement('option');option.value=String(index);option.textContent=source.label||`线路 ${index+1}`;select.appendChild(option)});if(choices.length<=1)select.hidden=true;
  ...
  toolbar.append(select,audioSelect,subSelect,subtitle,capture);shell.append(video,toolbar,status);root.replaceChildren(shell);load(0);
