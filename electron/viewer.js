...
if(kind==='video'){
  const shell=document.createElement('section'),video=document.createElement('video'),toolbar=document.createElement('div'),select=document.createElement('select'),speedSelect=document.createElement('select'),audioSelect=document.createElement('select'),subSelect=document.createElement('select'),subtitle=document.createElement('button'),capture=document.createElement('button'),status=document.createElement('div');shell.className='video-shell';toolbar.className='video-tools';video.controls=true;video.autoplay=true;video.playsInline=true;bindProgress(video);status.className='viewer-status';select.title='清晰度';speedSelect.title='播放速度';audioSelect.title='音轨';audioSelect.hidden=true;subSelect.title='在线字幕';subSelect.hidden=!subtitles.length;subtitle.textContent='字幕';capture.textContent='截图';const choices=sources.length?sources:[{url,label:'默认'}];choices.forEach((source,index)=>{const option=document.createElement('option');option.value=String(index);option.textContent=source.label||`线路 ${index+1}`;select.appendChild(option)});if(choices.length<=1)select.hidden=true;[.5,.75,1,1.25,1.5,2].forEach(rate=>{const option=document.createElement('option');option.value=String(rate);option.textContent=`${rate}×`;if(rate===1)option.selected=true;speedSelect.appendChild(option)});
  ...
  const load=(index,preserve=false)=>{const generation=++loadGeneration,snapshot=preserve?{time:Number(video.currentTime)||0,paused:video.paused,volume:video.volume,muted:video.muted,rate:video.playbackRate}:{time:0,paused:false,volume:video.volume,muted:video.muted,rate:Number(speedSelect.value)||1};current=index;showStatus(preserve?'正在切换清晰度…':'正在加载视频…');audioSelect.hidden=true;audioSelect.replaceChildren();if(hls){hls.destroy();hls=null}video.pause();video.removeAttribute('src');video.load();const restore=()=>{if(generation!==loadGeneration)return;if(snapshot.time>0&&Number.isFinite(video.duration))video.currentTime=Math.min(snapshot.time,Math.max(0,video.duration-.1));video.volume=snapshot.volume;video.muted=snapshot.muted;video.playbackRate=snapshot.rate;speedSelect.value=String(snapshot.rate);if(!snapshot.paused)video.play().catch(()=>{});showStatus(preserve?'清晰度已切换':'')};video.addEventListener('loadedmetadata',restore,{once:true});const source=choices[index]?.url||url,isHls=/\.m3u8(?:[?#]|$)|ts_downloader/i.test(source);if(isHls&&window.Hls?.isSupported()){const activeHls=hls=new window.Hls({enableWorker:true,manifestLoadingMaxRetry:2,fragLoadingMaxRetry:3});activeHls.loadSource(source);activeHls.attachMedia(video);activeHls.on(window.Hls.Events.MANIFEST_PARSED,()=>{if(generation===loadGeneration&&!preserve)video.play().catch(()=>{showStatus('点击播放按钮开始播放')})});activeHls.on(window.Hls.Events.AUDIO_TRACKS_UPDATED,(_event,data)=>{if(generation!==loadGeneration)return;const tracks=data.audioTracks||[];audioSelect.replaceChildren(...tracks.map((track,index)=>{const option=document.createElement('option');option.value=String(index);option.textContent=track.name||track.lang||`音轨 ${index+1}`;return option}));audioSelect.hidden=tracks.length<2});activeHls.on(window.Hls.Events.ERROR,(_event,data)=>{if(data.fatal)fail(generation)})}else{video.src=source;if(!preserve)video.play().catch(()=>{showStatus('点击播放按钮开始播放')})}};video.addEventListener('error',()=>{if(!hls)fail(loadGeneration)});select.addEventListener('change',()=>load(Number(select.value),true));speedSelect.addEventListener('change',()=>{video.playbackRate=Number(speedSelect.value)||1;showStatus(`播放速度 ${speedSelect.value}×`)});audioSelect.addEventListener('change',()=>{if(hls)hls.audioTrack=Number(audioSelect.value)});
