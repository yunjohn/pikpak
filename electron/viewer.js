...
if(kind==='video'){
  const shell=document.createElement('section'),video=document.createElement('video'),toolbar=document.createElement('div'),controls=document.createElement('div'),playToggle=document.createElement('button'),seek=document.createElement('input'),timeLabel=document.createElement('span'),volume=document.createElement('input'),fullscreen=document.createElement('button'),select=document.createElement('select'),speedSelect=document.createElement('select'),audioSelect=document.createElement('select'),subSelect=document.createElement('select'),previousVideo=document.createElement('button'),nextVideo=document.createElement('button'),autoNext=document.createElement('button'),subtitle=document.createElement('button'),pip=document.createElement('button'),external=document.createElement('button'),capture=document.createElement('button'),retryPlayback=document.createElement('button'),status=document.createElement('div');
  const preferenceKey='pikpak-viewer-video-preferences-v1';
  const readPreferences=()=>{try{const value=JSON.parse(localStorage.getItem(preferenceKey)||'{}');return {rate:[.5,.75,1,1.25,1.5,2].includes(Number(value.rate))?Number(value.rate):1,volume:Number.isFinite(Number(value.volume))?Math.max(0,Math.min(1,Number(value.volume))):1,muted:Boolean(value.muted),subtitleSize:[18,24,30,36].includes(Number(value.subtitleSize))?Number(value.subtitleSize):24,subtitleColor:['white','yellow','cyan'].includes(value.subtitleColor)?value.subtitleColor:'white',subtitleBackground:['dark','transparent'].includes(value.subtitleBackground)?value.subtitleBackground:'dark'}}catch{return {rate:1,volume:1,muted:false,subtitleSize:24,subtitleColor:'white',subtitleBackground:'dark'}}};
  const preferences=readPreferences(),savePreferences=()=>{try{localStorage.setItem(preferenceKey,JSON.stringify({rate:video.playbackRate,volume:video.volume,muted:video.muted,autoNext:autoNext.dataset.enabled==='1',subtitleSize:preferences.subtitleSize,subtitleColor:preferences.subtitleColor,subtitleBackground:preferences.subtitleBackground}))}catch{}};
  const savedAutoNext=(()=>{try{return JSON.parse(localStorage.getItem(preferenceKey)||'{}').autoNext===true}catch{return false}})();
  shell.className='video-shell';toolbar.className='video-tools';controls.className='video-controls';video.controls=false;video.autoplay=true;video.playsInline=true;video.volume=preferences.volume;video.muted=preferences.muted;video.playbackRate=preferences.rate;video.style.objectFit='contain';bindProgress(video);status.className='viewer-status';playToggle.textContent='▶';playToggle.title='播放 / 暂停';seek.type='range';seek.min='0';seek.max='1000';seek.value='0';seek.title='播放进度';volume.type='range';volume.min='0';volume.max='1';volume.step='.01';volume.value=String(video.volume);volume.title='音量';fullscreen.textContent='⛶';fullscreen.title='全屏';timeLabel.textContent='00:00 / 00:00';controls.append(playToggle,seek,timeLabel,volume,fullscreen);select.title='清晰度';speedSelect.title='播放速度';audioSelect.title='音轨';audioSelect.hidden=true;subSelect.title='在线字幕';subSelect.hidden=!subtitles.length;previousVideo.textContent='上一集';previousVideo.title='上一集（PageUp）';nextVideo.textContent='下一集';nextVideo.title='下一集（PageDown）';autoNext.dataset.enabled=savedAutoNext?'1':'0';autoNext.textContent=savedAutoNext?'自动连播：开':'自动连播：关';autoNext.title='播放结束后自动播放下一集';subtitle.textContent='字幕';pip.textContent='画中画';pip.title='画中画（P）';external.textContent='外部播放器';external.title='使用设置中选择的本机播放器打开';capture.textContent='截图';retryPlayback.textContent='重试播放';retryPlayback.title='重新获取播放地址并继续';retryPlayback.hidden=true;let choices=sources.length?sources:[{url,label:'默认'}];const renderChoices=()=>{select.replaceChildren(...choices.map((source,index)=>{const option=document.createElement('option');option.value=String(index);option.textContent=source.label||`线路 ${index+1}`;return option}));select.hidden=choices.length<=1};renderChoices();[.5,.75,1,1.25,1.5,2].forEach(rate=>{const option=document.createElement('option');option.value=String(rate);option.textContent=`${rate}×`;if(rate===preferences.rate)option.selected=true;speedSelect.appendChild(option)});
  video.setAttribute('aria-label',`${name} 视频播放器`);toolbar.setAttribute('role','toolbar');toolbar.setAttribute('aria-label','视频播放工具');controls.setAttribute('role','group');controls.setAttribute('aria-label','视频播放控制');status.setAttribute('role','status');status.setAttribute('aria-live','polite');[playToggle,seek,volume,fullscreen,select,speedSelect,audioSelect,subSelect,previousVideo,nextVideo,autoNext,subtitle,pip,external,capture,retryPlayback].forEach(control=>control.setAttribute('aria-label',control.title||control.textContent));
  ...
  const subtitleStyle=document.createElement('style');document.head.append(subtitleStyle);
  const applySubtitleStyle=()=>{const colors={white:'#fff',yellow:'#ffe66d',cyan:'#70f3ff'},background=preferences.subtitleBackground==='transparent'?'transparent':'rgba(0,0,0,.72)';subtitleStyle.textContent=`video::cue{font-size:${preferences.subtitleSize}px;color:${colors[preferences.subtitleColor]||'#fff'};background-color:${background}}`};applySubtitleStyle();
  const ensureDelayTools=()=>{
    ...
    const size=document.createElement('select');size.title='字幕字号';[18,24,30,36].forEach(value=>{const option=document.createElement('option');option.value=String(value);option.textContent=`字幕 ${value}px`;option.selected=value===preferences.subtitleSize;size.append(option)});size.addEventListener('change',()=>{preferences.subtitleSize=Number(size.value);applySubtitleStyle();savePreferences()});
    const color=document.createElement('select');color.title='字幕颜色';[['white','白色'],['yellow','黄色'],['cyan','青色']].forEach(([value,label])=>{const option=document.createElement('option');option.value=value;option.textContent=label;option.selected=value===preferences.subtitleColor;color.append(option)});color.addEventListener('change',()=>{preferences.subtitleColor=color.value;applySubtitleStyle();savePreferences()});
    const background=document.createElement('select');background.title='字幕背景';[['dark','深色背景'],['transparent','透明背景']].forEach(([value,label])=>{const option=document.createElement('option');option.value=value;option.textContent=label;option.selected=value===preferences.subtitleBackground;background.append(option)});background.addEventListener('change',()=>{preferences.subtitleBackground=background.value;applySubtitleStyle();savePreferences()});
    delayGroup.append(minus,plus,reset,size,color,background);toolbar.append(delayGroup);
  };
  ...
  retryPlayback.addEventListener('click',()=>{retryPlayback.hidden=true;refreshedMedia=false;current=Math.max(current,choices.length-1);fail(loadGeneration)});
  ...
  autoNext.setAttribute('aria-pressed',savedAutoNext?'true':'false');previousVideo.addEventListener('click',()=>switchEpisode(playlistIndex-1));nextVideo.addEventListener('click',()=>switchEpisode(playlistIndex+1));autoNext.addEventListener('click',()=>{const enabled=autoNext.dataset.enabled!=='1';autoNext.dataset.enabled=enabled?'1':'0';autoNext.textContent=enabled?'自动连播：开':'自动连播：关';autoNext.setAttribute('aria-pressed',enabled?'true':'false');savePreferences()});updateEpisodeButtons();
  ...
  const formatMediaTime=value=>{const seconds=Math.max(0,Math.floor(Number(value)||0)),hours=Math.floor(seconds/3600),minutes=Math.floor(seconds%3600/60),rest=seconds%60;return hours?`${hours}:${String(minutes).padStart(2,'0')}:${String(rest).padStart(2,'0')}`:`${String(minutes).padStart(2,'0')}:${String(rest).padStart(2,'0')}`};
  const syncControls=()=>{const duration=Number.isFinite(video.duration)?video.duration:0;seek.value=duration?String(Math.round(video.currentTime/duration*1000)):'0';timeLabel.textContent=`${formatMediaTime(video.currentTime)} / ${formatMediaTime(duration)}`;playToggle.textContent=video.paused?'▶':'❚❚';playToggle.setAttribute('aria-label',video.paused?'播放':'暂停');volume.value=String(video.muted?0:video.volume)};
  playToggle.addEventListener('click',()=>video.paused?video.play().catch(()=>{}):video.pause());seek.addEventListener('input',()=>{if(Number.isFinite(video.duration))video.currentTime=Number(seek.value)/1000*video.duration});volume.addEventListener('input',()=>{video.muted=false;video.volume=Number(volume.value)});fullscreen.addEventListener('click',toggleFullscreen);video.addEventListener('timeupdate',syncControls);video.addEventListener('durationchange',syncControls);video.addEventListener('play',syncControls);video.addEventListener('pause',syncControls);video.addEventListener('volumechange',syncControls);
  ...
  let gestureX=0,gestureY=0,gestureTimer=null,gestureLocked=false,videoScale=1;
  const zoomVideo=delta=>{videoScale=Math.max(.5,Math.min(3,videoScale+delta));video.style.transform=`scale(${videoScale})`;showControls();showStatus(`缩放 ${Math.round(videoScale*100)}%`)};
  shell.addEventListener('wheel',event=>{
    if(event.target?.closest?.('.video-tools,.video-controls'))return;
    if(event.ctrlKey){event.preventDefault();zoomVideo(Math.max(-.5,Math.min(.5,-event.deltaY*.01)));return}
    event.preventDefault();clearTimeout(gestureTimer);gestureTimer=setTimeout(()=>{gestureX=0;gestureY=0;gestureLocked=false},90);if(gestureLocked)return;gestureX+=event.deltaX;gestureY+=event.deltaY;
    if(Math.abs(gestureX)>80&&Math.abs(gestureX)>Math.abs(gestureY)){const seconds=gestureX>0?10:-10;video.currentTime=Math.max(0,Math.min(Number.isFinite(video.duration)?video.duration:Infinity,(video.currentTime||0)+seconds));showControls();showStatus(`${seconds>0?'快进':'后退'} ${Math.abs(seconds)} 秒`);gestureX=0;gestureY=0;gestureLocked=true}
    else if(Math.abs(gestureY)>80&&Math.abs(gestureY)>Math.abs(gestureX)){video.volume=Math.max(0,Math.min(1,video.volume+(gestureY<0?.05:-.05)));showControls();showStatus(`音量 ${Math.round(video.volume*100)}%`);gestureX=0;gestureY=0;gestureLocked=true}
  },{passive:false});
  window.addEventListener('beforeunload',()=>{clearTimeout(controlsTimer);clearTimeout(gestureTimer);subtitleStyle.remove();window.viewerPlayback?.setPlaying(false);if(subtitleUrl)URL.revokeObjectURL(subtitleUrl)});
  ...
  toolbar.append(previousVideo,nextVideo,autoNext,select,speedSelect,audioSelect,subSelect,subtitle,pip,external,capture,retryPlayback);shell.append(video,toolbar,status,controls);root.replaceChildren(shell);load(0);
  ...
  const preloadAround=center=>{
    ...
    for(let offset=-2;offset<=2;offset++){
      const item=gallery[center+offset];if(!item?.url)continue;
      wanted.add(item.url);
      if(!imageCache.has(item.url)){const preload=new Image();preload.src=item.url;imageCache.set(item.url,preload)}
    }
    for(const [cachedUrl,preload] of imageCache){if(!wanted.has(cachedUrl)){preload.src='';imageCache.delete(cachedUrl)}}
  };
  const button=(text,title,action)=>{const value=document.createElement('button');value.textContent=text;value.title=title;value.setAttribute('aria-label',title);value.addEventListener('click',action);tools.append(value);return value};
  ...
  const show=value=>{
    ...
    fit();image.src=item.url;image.alt=item.name;document.title=item.name;preloadAround(index);
  ...
  const slide=button('▶','开始幻灯片（空格）',()=>{
    if(timer){clearInterval(timer);timer=null;slide.textContent='▶';slide.title='开始幻灯片（空格）';slide.setAttribute('aria-label',slide.title);slide.setAttribute('aria-pressed','false')}
    else{timer=setInterval(()=>show(index+1<gallery.length?index+1:0),3000);slide.textContent='⏸';slide.title='暂停幻灯片（空格）';slide.setAttribute('aria-label',slide.title);slide.setAttribute('aria-pressed','true')}
  });
  ...
  window.addEventListener('beforeunload',()=>{if(timer)clearInterval(timer);clearTimeout(gestureTimer);for(const preload of imageCache.values())preload.src='';imageCache.clear()});
  ...
  if(!element){root.innerHTML='<div class="error">此文件类型暂不支持预览</div>'}else{
    if(element instanceof HTMLMediaElement){
      element.controls=true;element.autoplay=true;bindProgress(element);
      const shell=document.createElement('section'),status=document.createElement('div'),retryAudio=document.createElement('button');shell.className='audio-shell';status.className='viewer-status';status.setAttribute('role','status');status.setAttribute('aria-live','polite');retryAudio.className='playback-retry';retryAudio.textContent='重试播放';retryAudio.hidden=true;let gestureX=0,gestureY=0,gestureTimer=null,gestureLocked=false,refreshing=false,refreshed=false;
      const showGesture=message=>{status.textContent=message;setTimeout(()=>{if(status.textContent===message)status.textContent=''},1200)};
      shell.addEventListener('wheel',event=>{if(event.ctrlKey)return;event.preventDefault();clearTimeout(gestureTimer);gestureTimer=setTimeout(()=>{gestureX=0;gestureY=0;gestureLocked=false},90);if(gestureLocked)return;gestureX+=event.deltaX;gestureY+=event.deltaY;if(Math.abs(gestureX)>80&&Math.abs(gestureX)>Math.abs(gestureY)){const seconds=gestureX>0?10:-10;element.currentTime=Math.max(0,Math.min(Number.isFinite(element.duration)?element.duration:Infinity,(element.currentTime||0)+seconds));showGesture(`${seconds>0?'快进':'后退'} ${Math.abs(seconds)} 秒`);gestureX=0;gestureY=0;gestureLocked=true}else if(Math.abs(gestureY)>80&&Math.abs(gestureY)>Math.abs(gestureX)){element.volume=Math.max(0,Math.min(1,element.volume+(gestureY<0?.05:-.05)));showGesture(`音量 ${Math.round(element.volume*100)}%`);gestureX=0;gestureY=0;gestureLocked=true}},{passive:false});
      const refreshAudio=async()=>{if(refreshing||refreshed)return;refreshed=true;refreshing=true;retryAudio.hidden=true;const snapshot={time:Number(element.currentTime)||0,paused:element.paused,volume:element.volume,muted:element.muted,rate:element.playbackRate};try{status.textContent='播放地址可能已过期，正在刷新…';const fresh=await window.viewerPayload?.refreshMedia?.(currentFileId);const next=fresh?.url||fresh?.sources?.[0]?.url;if(!next)throw new Error('没有可用的音频播放地址');currentUrl=next;element.src=next;element.volume=snapshot.volume;element.muted=snapshot.muted;element.playbackRate=snapshot.rate;element.addEventListener('loadedmetadata',()=>{if(snapshot.time>0&&Number.isFinite(element.duration))element.currentTime=Math.min(snapshot.time,Math.max(0,element.duration-.1));if(!snapshot.paused)element.play().catch(()=>{});status.textContent='';status.classList.remove('error-status')},{once:true});element.load()}catch(error){status.textContent=`${error?.message||'音频播放失败'}；可点击重试`;status.classList.add('error-status');retryAudio.hidden=false}finally{refreshing=false}};
      element.addEventListener('waiting',()=>{status.textContent='正在缓冲…'});element.addEventListener('stalled',()=>{status.textContent='网络较慢，正在等待数据…'});element.addEventListener('playing',()=>{status.textContent='';window.viewerPlayback?.setPlaying(true)});element.addEventListener('pause',()=>window.viewerPlayback?.setPlaying(false));element.addEventListener('error',refreshAudio);
      retryAudio.addEventListener('click',()=>{refreshed=false;refreshAudio()});window.addEventListener('beforeunload',()=>{clearTimeout(gestureTimer);window.viewerPlayback?.setPlaying(false)});element.src=currentUrl;element.title=name;shell.append(element,status,retryAudio);root.replaceChildren(shell);
    }else{element.src=url;element.title=name;element.addEventListener('error',()=>{root.innerHTML='<div class="error">文件打开失败，请检查登录状态或链接是否已过期</div>'});root.replaceChildren(element)}
  }
}
}
