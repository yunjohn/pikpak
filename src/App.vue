...
const title=computed(()=>mode.value==='drive'?'我的 PikPak':mode.value==='search'?'全盘搜索':mode.value==='external-search'?'外部搜索':mode.value==='starred'?'收藏':mode.value==='recent'?'最近':mode.value==='myshares'?'我的分享':mode.value==='transfers'?'传输中心':mode.value==='trash'?'回收站':mode.value==='settings'?'设置':(share.value?'分享文件':'打开分享'));
...
function showSearchPage(){invalidatePageRequest();searchGeneration++;mode.value='search';files.value=[];searchStats.value=null;error.value='';pathStack.value=[{id:'search',name:'全盘搜索'}];clearSelection()}
...
async function addExternal(result){await run(async()=>{await api.addExternalMagnet(result.magnet);await refreshOfflineTasks();notice.value=`已添加“${result.title}”到离线下载`;setTimeout(()=>{notice.value=''},3500)})}
  ...
  <div class="shell">
    <aside class="sidebar">
      ...
      <nav>
        ...
        <button :class="{active:mode==='search'}" title="全盘搜索" @click="showSearchPage"><span>⌕</span> 全盘搜索</button>
        <button :class="{active:mode==='external-search'}" @click="showExternalSearch"><span>◎</span> 外部搜索</button>
        <button :class="{active:mode==='starred'}" :disabled="!account.connected" :title="account.connected?'查看收藏':'连接账号后可用'" @click="loadStarred(true)"><span>★</span> 收藏</button>
        <button :class="{active:mode==='recent'}" :disabled="!account.connected" :title="account.connected?'查看最近文件':'连接账号后可用'" @click="loadRecent(true)"><span>◷</span> 最近</button>
        <button :class="{active:mode==='myshares'}" :disabled="!account.connected" :title="account.connected?'查看我的分享':'连接账号后可用'" @click="loadMyShares(true)"><span>♢</span> 我的分享</button>
        ...
        <button :class="{active:mode==='trash'}" :disabled="!account.connected" :title="account.connected?'查看回收站':'连接账号后可用'" @click="loadTrash(true)"><span>♲</span> 回收站</button>
      ...
      <button class="account-card" :disabled="loading" @click="account.connected?logout():webLogin()"><span :class="['dot',{online:account.connected}]"></span>{{loading?'正在处理…':account.connected?'账户已连接 · 退出':'网页登录 PikPak'}}</button>
    </aside>
    ...
    <main class="main">
      <header class="toolbar">
        <div class="toolbar-top">
          ...
          <div class="toolbar-actions">
              ...
              <button v-if="(mode==='drive'&&account.connected)||(mode==='share'&&share)" class="soft" :disabled="loading" @click="refreshCurrent">{{loading?'刷新中…':'↻ 刷新'}}</button>
              <button v-if="mode==='starred'" class="soft" :disabled="loading" @click="loadStarred">{{loading?'刷新中…':'↻ 刷新'}}</button>
              <button v-if="mode==='recent'" class="soft" :disabled="loading" @click="loadRecent">{{loading?'刷新中…':'↻ 刷新'}}</button>
              <button v-if="mode==='myshares'" class="soft" :disabled="loading" @click="loadMyShares">{{loading?'刷新中…':'↻ 刷新'}}</button>
              <button v-if="mode==='trash'" class="soft" :disabled="loading" @click="loadTrash">{{loading?'刷新中…':'↻ 刷新'}}</button>
              <button v-if="mode==='transfers'&&account.connected" class="soft" :disabled="loading" @click="refreshOfflineTasks">{{loading?'刷新中…':'↻ 刷新'}}</button>
        ...
        <div class="toolbar-sub">
          <p v-if="mode==='search'" class="toolbar-description">在整个网盘中查找文件和文件夹</p><p v-else-if="mode==='external-search'" class="toolbar-description">搜索公开资源，并可直接保存到 PikPak</p><div v-else class="crumbs"><template v-for="(part,i) in pathStack" :key="part.id"><button class="crumb-btn" :title="part.name" @click="navigateCrumb(i)">{{part.name}}</button><span v-if="i<pathStack.length-1" class="crumb-sep">›</span></template></div>
        </div>
      </header>
      ...
      <section v-if="mode==='external-search'" class="external-search-panel"><div class="external-search-head"><div class="search-page-intro"><span class="search-page-icon">◎</span><div><h2>搜索外部资源</h2><small>结果由 u9a9.org 提供，可复制磁力链接或保存到 PikPak。</small></div></div><form @submit.prevent="searchExternal"><input v-model="externalQuery" autofocus aria-label="外部资源搜索关键词" placeholder="输入资源名称，至少 2 个字符"><button class="primary" :disabled="loading||externalQuery.trim().length<2">{{loading?'搜索中…':'搜索'}}</button></form><p v-if="error" class="search-page-error error" role="alert">{{error}}</p></div><div class="external-results"><div v-if="!loading&&!externalResults.length" class="state search-empty"><span>⌕</span><b>输入关键词开始搜索</b><small>搜索结果可直接复制磁力链接</small></div><article v-for="result in externalResults" :key="result.key" class="external-result"><div><b :title="result.title">{{result.title}}</b><small :title="externalMagnetSummary(result.magnet)">{{externalMagnetSummary(result.magnet)}}</small></div><button class="soft" @click="copyExternal(result)">复制磁力</button><button class="primary" :disabled="!account.connected" @click="addExternal(result)">保存到网盘</button></article></div></section>
      <section v-else-if="mode==='search'&&!searchStats" class="connect-panel search-all-panel"><div class="search-page-icon">⌕</div><h2>{{account.connected?'搜索整个 PikPak':'连接账户后使用全盘搜索'}}</h2><template v-if="account.connected"><p :class="{error}" :role="error?'alert':'status'" aria-live="polite">{{error|| (loading?`正在遍历目录，已扫描 ${searchLive.folders} 个目录、${searchLive.scanned} 项，找到 ${searchLive.matchesCount} 项…`:'后台遍历网盘目录，并在结果中保留文件所在路径。')}}</p><form @submit.prevent="searchAll"><input v-model="globalQuery" autofocus aria-label="全盘搜索关键词" placeholder="输入文件或文件夹名称，至少 2 个字符"><button class="primary" :disabled="loading||globalQuery.trim().length<2">{{loading?'搜索中…':'开始搜索'}}</button><button v-if="loading" type="button" class="soft" @click="cancelSearch">取消搜索</button></form></template><template v-else><p>连接 PikPak 账户后，可搜索网盘内的全部文件和文件夹。</p><button class="primary search-login" :disabled="loading" @click="webLogin">{{loading?'正在连接…':'连接 PikPak 账户'}}</button></template></section>
      ...
      <section v-else class="content">
        <div ref="filePanelEl" :class="['file-panel',{'drop-active':dragUpload}]" role="region" :aria-label="`${title}文件列表`" @mousedown="onFilePanelMouseDown" @dragstart.prevent @scroll.passive="onListScroll" @dragenter.prevent="mode==='drive'&&account.connected&&(dragUpload=true)" @dragover.prevent @dragleave="leaveDrop" @drop.prevent="dropFiles">
          ...
          <div v-if="loading&&!files.length" class="state" role="status" aria-live="polite">正在加载…</div><div v-else-if="error&&!files.length" class="state state-retry error" role="alert"><span>{{error}}</span><button v-if="['drive','share','starred','recent','myshares','trash'].includes(mode)" class="soft" @click="refreshCurrent">重试</button></div><div v-else-if="!files.length" class="state" role="status">这个目录是空的</div>
          <div v-if="loading&&files.length" class="list-feedback" role="status" aria-live="polite"><span>正在刷新当前列表…</span></div><div v-else-if="error&&files.length" class="list-feedback error" role="alert"><span>{{error}}</span><button class="soft" @click="refreshCurrent">重试</button></div>
