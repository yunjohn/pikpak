...
function restoreNavigationState(){try{const value=JSON.parse(localStorage.getItem(NAV_STATE_KEY)||'null');if(!value||typeof value!=='object')return;if(Array.isArray(value.path)&&value.path.length&&value.path.length<100)pathStack.value=value.path.map(item=>({id:String(item.id||''),name:String(item.name||'目录').slice(0,200)}));if(Array.isArray(value.expanded))treeExpanded.value=value.expanded.map(String).filter(Boolean).slice(0,1000);if(value.children&&typeof value.children==='object'&&!Array.isArray(value.children)){const safe={};for(const [id,nodes] of Object.entries(value.children).slice(0,1000))if(Array.isArray(nodes))safe[id]=nodes.slice(0,1000).map(node=>({id:String(node.id||''),name:String(node.name||'目录').slice(0,200),kind:'drive#folder'})).filter(node=>node.id);treeChildren.value=safe}}catch{localStorage.removeItem(NAV_STATE_KEY)}}
function persistNavigationState(){if(mode.value!=='drive')return;try{localStorage.setItem(NAV_STATE_KEY,JSON.stringify({path:pathStack.value,expanded:treeExpanded.value,children:treeChildren.value}))}catch{}}
...
async function saveToken(){account.value=await api.setAccessToken(token.value.trim());token.value='';if(account.value.connected){refreshQuota();resumeDrive()}}
async function webLogin(){await run(async()=>{account.value=await api.login();if(account.value.connected){await Promise.all([refreshQuota(),resumeDrive()])}})}
async function logout(){await run(async()=>{account.value=await api.logout();quota.value=null;files.value=[];treeChildren.value={};treeExpanded.value=[];clearSelection();pathStack.value=[{id:'',name:'全部文件'}];localStorage.removeItem(NAV_STATE_KEY)})}
...
onMounted(async()=>{restoreNavigationState();api.onDownload(upsertDownload);api.onUpload(upsertUpload);api.onAccountExpired(()=>{account.value={connected:false};quota.value=null;files.value=[];clearSelection()});[downloads.value,uploads.value,settings.value]=await Promise.all([api.listDownloads(),api.listUploads(),api.getSettings()]);account.value=await api.getAccount();if(account.value.connected){refreshQuota();resumeDrive()}})
</script>
